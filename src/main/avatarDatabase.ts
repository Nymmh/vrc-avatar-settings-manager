...
export function avatarDatabase(log: Logger): DBType {
  ...
  if (version === 4) {
    log.info('Upgrading Meow Storage to version 5...')
  ...
  if (version === 6) {
    log.info('Upgrading Meow Storage to version 7...')

    db.transaction(() => {
      db.prepare(
        `INSERT OR IGNORE INTO settings (key, value) VALUES ('tiplinkWebhookSecret', '')`
      ).run()
      db.pragma('user_version = 7')
    })()

    version = 7
    log.info('Meow Storage upgraded to version 7')
  }

  if (version === 7) {
    log.info('Upgrading Meow Storage to version 8...')

    db.transaction(() => {
      db.prepare(
        `INSERT OR IGNORE INTO settings (key, value) VALUES ('tiplinkWebhookPreviousSecret', ''), ('tiplinkWebhookPreviousSecretExpiresAt', '')`
      ).run()
      db.pragma('user_version = 8')
    })()

    version = 8
    log.info('Meow Storage upgraded to version 8')
  }
